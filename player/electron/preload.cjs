...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  cache: {
    /**
     * 진행률 이벤트 포함 다운로드 (프로그레스바용)
     * @returns {{ success, path, cached, size } | { success: false, error }}
     */
    downloadWithProgress: (url, contentId, filename) =>
      ipcRenderer.invoke('cache:downloadWithProgress', { url, contentId, filename }),

    /**
     * 진행률 이벤트 구독
     * callback: ({ contentId, received, total, percent }) => void
     * @returns {Function} unsubscribe 함수
     */
    onDownloadProgress: (callback) => {
      const handler = (_event, data) => callback(data);
      ipcRenderer.on('cache:downloadProgress', handler);
      return () => ipcRenderer.removeListener('cache:downloadProgress', handler);
    },
